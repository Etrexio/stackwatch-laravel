...
### Fixed
- **Fixed `--no-interaction` option conflict** in `stackwatch:install` command - Removed duplicate `--no-interaction` option from command signature that conflicted with Symfony Console's built-in option, causing "An option named no-interaction already exists" error
